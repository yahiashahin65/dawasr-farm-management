...
import { faTractor, faUsers, faWheatAwn, faCircleCheck, faTriangleExclamation, faCircleXmark } from "@fortawesome/free-solid-svg-icons";

export default function Dashboard(){
 const [assets,setAssets]=useState([]); const [workers,setWorkers]=useState([]); const [farms,setFarms]=useState([]);
 useEffect(()=>{getDocs(collection(db,"assets")).then(s=>setAssets(s.docs.map(d=>({id:d.id,...d.data()})))); getDocs(collection(db,"workers")).then(s=>setWorkers(s.docs.map(d=>({id:d.id,...d.data()})))); getDocs(collection(db,"farms")).then(s=>setFarms(s.docs.map(d=>({id:d.id,...d.data()}))));},[]);
