...
const typeLabel = (type) =>
  ({ worker: "عامل", engineer: "مهندس", accountant: "محاسب" }[type] || "-");

const removeVehicleFromCache = (vehicleId) =>
  setCachedCollection(
    "cache:vehicles",
    getCachedCollection("cache:vehicles").filter((item) => item.id !== vehicleId)
  );
...
export default function Vehicles() {
  ...
  useEffect(() => {
    const unsubscribeVehicles = subscribeCachedCollection({
      ...
      onError: () => setRealtimeError("تعذر تحديث بيانات السيارات لحظيًا"),
    });
  ...
  const filteredItems = useMemo(() => {
    ...
    return items.filter((vehicle) => {
      if (statusFilter === "صالح" && !isAvailableStatus(vehicle.status)) {
        return false;
      }
      ...
      const haystack = `${vehicle.name || ""} ${vehicle.plateLetters || ""} ${
        vehicle.plateNumbers || ""
      } ${vehicle.assignedToName || ""} ${vehicle.farmName || ""}`.toLowerCase();
  ...
  const enterWorkshop = async (e) => {
    ...
    try {
      const entryPayload = {
        vehicleId: entryVehicle.id,
        vehicleName: entryVehicle.name || "",
        plateLetters: entryVehicle.plateLetters || "",
        plateNumbers: entryVehicle.plateNumbers || "",
        workshopName: entryForm.workshopName.trim(),
        faultReason: entryForm.faultReason.trim(),
        enteredAt: entryForm.enteredAt,
        exitedAt: "",
        status: "in_workshop",
        repairCost: 0,
        hasSpareParts: false,
      ...
      const maintenanceRef = await addDoc(
        collection(db, "vehicleMaintenance"),
        entryPayload
      );

      await updateDoc(doc(db, "vehicles", entryVehicle.id), {
        status: "in_workshop",
        workshopEntryId: maintenanceRef.id,
        workshopName: entryForm.workshopName.trim(),
        faultReason: entryForm.faultReason.trim(),
        workshopEnteredAt: entryForm.enteredAt,
        updatedAt: serverTimestamp(),
      });
      ...
      console.error(error);
      alert("تعذر إدخال السيارة الورشة");
    } finally {
      setSavingAction(false);
    }
  };

  const openExit = (vehicle) => {
  ...
  const addPart = () =>
    setExitForm((prev) => ({
      ...prev,
      spareParts: [...prev.spareParts, { name: "", price: "" }],
    }));
  ...
  const calculatedTotals = useMemo(() => {
    ...
    const parts =
      exitForm.hasSpareParts === "yes"
        ? exitForm.spareParts.filter((part) => part.name.trim() || part.price)
        : [];

    const totalSparePartsCost = parts.reduce(
    ...
    return {
      ...
      parts,
      totalSparePartsCost,
      totalCost: repairCost + totalSparePartsCost,
    };
  }, [exitForm]);
  ...
  const exitWorkshop = async (e) => {
    ...
    try {
      ...
      if (maintenanceId) {
        await updateDoc(doc(db, "vehicleMaintenance", maintenanceId), {
          exitedAt: exitForm.exitedAt,
          status: "completed",
          repairCost: calculatedTotals.repairCost,
          hasSpareParts: exitForm.hasSpareParts === "yes",
          spareParts: calculatedTotals.parts,
          totalSparePartsCost: calculatedTotals.totalSparePartsCost,
          totalCost: calculatedTotals.totalCost,
          invoicePaid,
          invoiceStatus: invoicePaid ? "paid" : "unpaid",
          exitNotes: exitForm.notes.trim(),
          updatedAt: serverTimestamp(),
        });
      } else {
        await addDoc(collection(db, "vehicleMaintenance"), {
          vehicleId: exitVehicle.id,
          vehicleName: exitVehicle.name || "",
          plateLetters: exitVehicle.plateLetters || "",
          plateNumbers: exitVehicle.plateNumbers || "",
          enteredAt: exitVehicle.workshopEnteredAt || "",
          exitedAt: exitForm.exitedAt,
          status: "completed",
          repairCost: calculatedTotals.repairCost,
          hasSpareParts: exitForm.hasSpareParts === "yes",
      ...
      await updateDoc(doc(db, "vehicles", exitVehicle.id), {
        ...
        faultReason: "",
        workshopEnteredAt: "",
        lastMaintenanceCost: calculatedTotals.totalCost,
        lastMaintenancePaid: invoicePaid,
        lastMaintenanceExitedAt: exitForm.exitedAt,
        updatedAt: serverTimestamp(),
      });
      ...
      <Layout title="إدارة السيارات">
        {initialLoading ? (
          <AppLoader
            ...
            subtitle="يتم تجهيز بيانات السيارات"
          />
        ) : (
          <>
            {realtimeError && (
            ...
            <div className="mb-4 grid gap-3 md:grid-cols-4">
              ...
              <div className="page-card p-5">
                <p className="text-sm font-bold text-slate-500">صالحة</p>
              ...
              <div className="page-card p-5">
                <p className="text-sm font-bold text-slate-500">في الورشة</p>
                <h3 className="mt-2 text-4xl font-black text-amber-700">
                  {stats.inWorkshop}
                </h3>
              </div>
            ...
            <div className="mb-4 flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
              <div className="page-card flex flex-1 items-center gap-2 p-3">
                ...
                <input
                  ...
                  placeholder="بحث باسم السيارة أو الراكب أو المزرعة أو حروف/أرقام اللوحة..."
                  value={search}
                  onChange={(e) => {
                    ...
                    <tr key={vehicle.id} className="border-t border-slate-100">
                      ...
                      <td className="table-td">
                        {vehicle.assignedToName
                          ? `${vehicle.assignedToName} (${typeLabel(
                              vehicle.assignedToType
                            )})`
                          : "اختياري / غير محدد"}
                      </td>

                      <td className="table-td">
                        {vehicle.farmName || "اختياري / غير محدد"}
                      </td>

                      <td className="table-td">
                        <span className={`badge ${statusClass(vehicle.status)}`}>
                      ...
                      <td className="table-td">
                        <div className="flex flex-wrap gap-2">
                          ...
                          {canManage && vehicle.status !== "in_workshop" && (
                            <button
                              onClick={() => openEntry(vehicle)}
                              className="btn-secondary !py-2"
                            >
            ...
            <div className="mt-4 flex items-center justify-center gap-2">
              ...
              <span className="rounded-2xl bg-white px-4 py-2 text-sm font-black shadow">
                {currentPage} / {totalPages}
              </span>
        ...
        {entryVehicle && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
            <form
              onSubmit={enterWorkshop}
              className="page-card w-full max-w-2xl space-y-4 p-5"
            >
              <h3 className="text-xl font-black">
                إدخال السيارة الورشة: {entryVehicle.name}
              </h3>

              <input
                className="form-input"
                placeholder="اسم الورشة اختياري"
                value={entryForm.workshopName}
                onChange={(e) =>
                  setEntryForm({ ...entryForm, workshopName: e.target.value })
                }
              />
              ...
              <input
                className="form-input"
                type="date"
                value={entryForm.enteredAt}
                onChange={(e) =>
                  setEntryForm({ ...entryForm, enteredAt: e.target.value })
                }
              />
              ...
              <textarea
                ...
                value={entryForm.faultReason}
                onChange={(e) =>
                  setEntryForm({ ...entryForm, faultReason: e.target.value })
                }
              />
              ...
              <div className="flex gap-2">
                <button disabled={savingAction} className="btn-primary">
                  تأكيد الإدخال
                </button>

                <button
                  type="button"
                  onClick={() => setEntryVehicle(null)}
                  className="btn-secondary"
                >
                  إلغاء
                </button>
        ...
        {exitVehicle && (
          <div className="fixed inset-0 z-50 flex items-center justify-center overflow-y-auto bg-black/40 p-4">
            <form
              onSubmit={exitWorkshop}
              className="page-card my-8 w-full max-w-3xl space-y-4 p-5"
            >
              <h3 className="text-xl font-black">
                خروج السيارة من الورشة: {exitVehicle.name}
              </h3>
              ...
              <div className="grid gap-4 md:grid-cols-2">
                ...
                <input
                  ...
                  type="number"
                  min="0"
                  placeholder="سعر الصيانة الأساسي"
                  value={exitForm.repairCost}
                  onChange={(e) =>
                    setExitForm({ ...exitForm, repairCost: e.target.value })
                  }
                />

                <select
                  className="form-input"
                  value={exitForm.hasSpareParts}
                  onChange={(e) =>
                    setExitForm({ ...exitForm, hasSpareParts: e.target.value })
                  }
                >
                  <option value="no">لا يوجد قطع غيار</option>
                  <option value="yes">يوجد قطع غيار</option>
                </select>

                <select
                  className="form-input"
                  value={exitForm.invoicePaid}
                  onChange={(e) =>
                    setExitForm({ ...exitForm, invoicePaid: e.target.value })
                  }
                >
                  <option value="no">الفاتورة غير مسددة</option>
                  <option value="yes">الفاتورة مسددة</option>
                </select>
              </div>
              ...
              {exitForm.hasSpareParts === "yes" && (
                <div className="rounded-2xl bg-slate-50 p-4">
                  <div className="mb-3 flex items-center justify-between">
                    <h4 className="font-black">قطع الغيار</h4>
                    <button
                      type="button"
                      onClick={addPart}
                      className="btn-secondary !py-2"
                  ...
                  <div className="space-y-2">
                    {exitForm.spareParts.map((part, index) => (
                      <div
                        key={index}
              ...
              <div className="grid gap-3 md:grid-cols-3">
                <div className="rounded-2xl bg-green-50 p-4 font-black text-green-700">
                  الصيانة: {calculatedTotals.repairCost}
                </div>

                <div className="rounded-2xl bg-blue-50 p-4 font-black text-blue-700">
                  قطع الغيار: {calculatedTotals.totalSparePartsCost}
                </div>

                <div className="rounded-2xl bg-slate-900 p-4 font-black text-white">
                  الإجمالي: {calculatedTotals.totalCost}
                </div>
              </div>
              ...
              <textarea
                ...
                onChange={(e) =>
                  setExitForm({ ...exitForm, notes: e.target.value })
                }
              />

              <div className="flex gap-2">
                <button disabled={savingAction} className="btn-primary">
                  تسجيل الخروج والفاتورة
                </button>

                <button
                  type="button"
                  onClick={() => setExitVehicle(null)}
                  className="btn-secondary"
                >
                  إلغاء
                </button>
