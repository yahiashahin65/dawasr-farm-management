...
export default function Farms(){const [items,setItems]=useState([]); const load=async()=>{const s=await getDocs(query(collection(db,"farms"),orderBy("createdAt","desc"))); setItems(s.docs.map(d=>({id:d.id,...d.data()})));}; useEffect(()=>{load();},[]); const remove=async(id)=>{if(confirm("حذف المزرعة؟")){await deleteDoc(doc(db,"farms",id)); load();}};
return (
  <ProtectedRoute>
    <Layout title="المزارع">
      {/* تم حذف الفقرة النصية وتعديل المحاذاة لزر الإضافة */}
