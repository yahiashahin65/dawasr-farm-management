...
// نفس منطق صفحة الأصول والفلاتر بالضبط
const filterAssetsByStatus = (assets, status) =>
  assets.filter((asset) => asset.status === status);

const filterAssetsByCategory = (assets, category) =>
  assets.filter((asset) => (asset.category || "asset") === category);

const filterAssetsByPlaceType = (assets, placeType) =>
  assets.filter((asset) => asset.placeType === placeType);
...
const actions = {
  ...
  "اعرض الأصول العاطلة": (data) => {
    const list = filterAssetsByStatus(data.assets, "عاطل");
    return result({
  ...
  "اعرض الأصول في الورشة": (data) => {
    // لا نعتمد على اسم المكان نهائيًا عشان كِبرة اسمها ورشة الحوطي
    const list = filterAssetsByStatus(data.assets, "في الورشة");
    return result({
  ...
  "اعرض الأصول الصالحة": (data) => {
    const list = filterAssetsByStatus(data.assets, "صالح");
    return result({
  ...
  "اعرض الأصول داخل المزارع": (data) => {
    const list = filterAssetsByPlaceType(data.assets, "farm");
    return result({
  ...
  "اعرض الأصول داخل الكِبر": (data) => {
    const list = filterAssetsByPlaceType(data.assets, "kubra");
    return result({
  ...
  "اعرض ملخص الأصول": (data) => {
    ...
    return result({
      answer: `ملخص الأصول: الإجمالي ${stats.total}، الصالح ${stats.good}، العاطل ${stats.broken}، في الورشة ${stats.inWorkshop}، معدات ${stats.equipment}، قطع غيار ${stats.spareParts}، أدوات ${stats.tools}، مواد ${stats.materials}، داخل المزارع ${stats.inFarms}، داخل الكِبر ${stats.inKubras}، في الورش ${stats.inExternalWorkshops}.`,
      items: [
        { title: "كل الأصول", subtitle: `${stats.total} أصل`, href: "/assets" },
        { title: "الأصول العاطلة", subtitle: `${stats.broken} أصل`, href: "/assets?status=عاطل" },
        { title: "الأصول في الورشة", subtitle: `${stats.inWorkshop} أصل`, href: "/assets?status=في الورشة" },
