...
// IMPORTANT:
// These match Dashboard / Assets page logic exactly.
// Do not infer from place name, because some kubras can contain the word "ورشة".
...
// Supports dashboard logic (workerIds) + fallback by workerNames because your table displays workerNames.
...
// Matches Dashboard / Assets filters.
...
const actions = {
  ...
  "اعرض الأصول داخل المزارع": (data) => {
    ...
    return result({
      answer: `عدد الأصول داخل المزارع: ${stats.inFarms}.`,
      items: list.slice(0, 12).map(mapAsset),
    });
  },
  ...
  "اعرض الأصول داخل الكِبر": (data) => {
    ...
    return result({
      answer: `عدد الأصول داخل الكِبر: ${stats.inKubras}.`,
      items: list.slice(0, 12).map(mapAsset),
    });
  },
