...
import { getAssetTypeName, getPlaceName } from "./inventory";
...
const isAssetInWorkshop = (asset) => asset.status === "في الورشة";
const isAssetInFarm = (asset) => asset.placeType === "farm";
const isAssetInKubra = (asset) => asset.placeType === "kubra";
...
const getAssetsByWorker = (assets, workerId, workerName) =>
  assets.filter((asset) => {
    ...
    if (
      asset.workerNames &&
      workerName &&
      normalize(asset.workerNames).includes(normalize(workerName))
    ) {
      return true;
    }
...
const getFarmSprinklers = (sprinklers, farm) =>
  sprinklers.filter(
    (sprinkler) =>
      sprinkler.farmId === farm.id || sprinkler.farmName === farm.name
  );
...
const systemSummary = (data) => {
  ...
  return result({
    answer: `ملخص النظام: ${stats.total} أصل، ${stats.good} صالح، ${stats.broken} عاطل، ${stats.inWorkshop} في الورشة، ${data.workers.length} عامل، ${data.farms.length} مزرعة، ${data.kubras.length} كِبرة، ${data.sprinklers.length} رشاش، ${data.heaps.length} كوم، وإجمالي اللبن ${totalBricks}.`,
    items: [
      {
        title: "صفحة التقارير",
        subtitle: "عرض التقارير كاملة",
        href: "/reports",
      },
      {
        title: "صفحة التحليلات",
        subtitle: "عرض الرسومات والتحليلات",
        href: "/analytics",
      },
    ],
  });
...
const actions = {
  "كم عدد كل الأصول؟": (data) => {
    ...
    return result({
      ...
      items: [
        {
          title: "كل الأصول",
          subtitle: `${stats.total} أصل`,
          href: "/assets",
        },
        {
          title: "الأصول العاطلة",
          subtitle: `${stats.broken} أصل`,
          href: "/assets?status=عاطل",
        },
        {
          title: "الأصول في الورشة",
          subtitle: `${stats.inWorkshop} أصل`,
  ...
  "مين أكثر عامل ماسك عهد؟": (data) => {
    const rows = data.workers
      .map((worker) => ({
        worker,
        count: getAssetsByWorker(data.assets, worker.id, worker.name).length,
      }))
    ...
    return result({
      answer: `أكثر عامل ماسك عهد هو ${
        rows[0].worker.name || "بدون اسم"
      } بعدد ${rows[0].count} أصل.`,
  ...
  "اعرض المزارع وعدد الرشاشات": (data) => {
    const rows = data.farms
      .map((farm) => ({
        farm,
        count: getFarmSprinklers(data.sprinklers, farm).length,
      }))
      .sort((a, b) => b.count - a.count);
  ...
  "اعرض الكِبر وعدد الأصول": (data) => {
    const rows = data.kubras
      .map((kubra) => ({
        kubra,
        count: getKubraAssets(data.assets, kubra).length,
      }))
      .sort((a, b) => b.count - a.count);
...
export const askSmartAssistant = (message, liveData = null) => {
  const question = String(message || "").trim();

  const cachedData = getCollections();

  const data = {
    assets: liveData?.assets || cachedData.assets || [],
    workers: liveData?.workers || cachedData.workers || [],
    engineers: liveData?.engineers || cachedData.engineers || [],
    farms: liveData?.farms || cachedData.farms || [],
    kubras: liveData?.kubras || cachedData.kubras || [],
    sprinklers: liveData?.sprinklers || cachedData.sprinklers || [],
    heaps: liveData?.heaps || cachedData.heaps || [],
    movements: liveData?.movements || cachedData.movements || [],
  };

  if (!question) return fallback();

  const directKey = Object.keys(actions).find((key) =>
    sameQuestion(key, question)
  );
