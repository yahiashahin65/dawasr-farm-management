...
// Important: these must match pages/assets and dashboard logic exactly.
// Do not infer by place name because a Kubra can be named "ورشة ...".
...
// Same worker logic used in dashboard/pages: asset.workerIds includes worker.id
const getAssetsByWorker = (assets, workerId) =>
  assets.filter((asset) => (asset.workerIds || []).includes(workerId));
